                                    ...
                                    <td width="150">
                                        <div class="btn-group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="/disk/open?_=${disk.id}">打开</a></li>
...
<script>
    $(function () {
        ...
        var uploader = WebUploader.create({
            ...
            auto:true, //自动上传
            fileVal:'file',  //发送给服务器的数据，文件上传在后台接收文件时所用的名称
            formData:{
                "pId":pid,
                "accountId":accountId
            } //发送请求给服务器的额外数据
        ...
        //重命名...标签中的属性要用calss（如果用id只对单个有用）
        $(".rename").click(function () {
            var id = $(this).attr("rel");
            //事件委托，停止事件冒泡
            event.stopPropagation();
            //prompt层  ...pass输入的内容...index用于关闭
            layer.prompt({title: '请输入文件名，并确认'}, function(pass, index){
                layer.close(index);
                $.post("/disk/rename",{'id':id,'newFileName':pass}).done(function (data) {
        ...
        $(".delete").click(function () {
            ...
            event.stopPropagation();
            //layer弹出层
            layer.confirm("确定要删除码？",function () {
                $.post("/disk/delete",{'id':id}).done(function (data) {
